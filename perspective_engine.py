...
class AutoPerspectiveEngine:
    ...
    def _color_candidate(self, image):
        """
        Bantuan untuk KTP berwarna cyan/biru.
        Candidate ini tidak otomatis menang; tetap ikut scoring umum.
        """
        hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)

        # Rentang sengaja cukup konservatif agar background abu-abu/laptop
        # tidak mudah ikut menjadi badan kartu.
        mask = cv2.inRange(
            hsv,
            np.array([78, 30, 55], dtype=np.uint8),
        ...
        for contour in sorted(
            ...
            for eps in (0.012, 0.020, 0.030, 0.045, 0.060):
                approx = cv2.approxPolyDP(
                    hull,
                    eps * perimeter,
                    True,
    ...
    def _candidate_score(self, candidate, image_shape, edge_map):
        h, w = image_shape[:2]
    ...
    def _warp_quality(self, warped):
        ...
        # Ada isi/variasi gambar; kandidat kosong/background mendapat nilai rendah.
        contrast_score = min(
        ...
        # Dokumen identitas biasanya memiliki teks/detail yang cukup.
        detail_score = min(
            edge_density / 0.12,
            1.0,
        )
